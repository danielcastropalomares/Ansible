...
Ejemplo para de como utilizar las variables en el skeleton:
```
---
- hosts: zabbix
  become: yes
  roles:
     - ssl
     - minimal-system
     - { role: zabbix-server, zabbix_server_database: mysql, zabbix_server_database_long: mysql, zabbix_server_dbport: 3306, zabbix_server_dbpassword: XXXXXX }
     - { role: secure-mysql, mysql_root_password: XXXXXXX }
```
Este plyabook incluye:

* Instalación del Zabbix server - (https://github.com/dj-wasabi/ansible-zabbix-server)
* Instalación Zabbix gui
* Partioning MYSQL
* Sysmondash 1.1
* Incluye los script echo.something y if.speed
* Instalación mibs y snmp
* Habilitado el SSL con certificado autofirmado.
